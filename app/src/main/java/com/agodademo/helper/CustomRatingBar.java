...
public class CustomRatingBar extends View {
    ...
    public enum Gravity {
        ...
        static Gravity fromId(int id) {
            ...
            // default value
            LogUtil.w("SimpleRatingBar", String.format("Gravity chosen is neither 'left' nor 'right', I will set it to Left"));
            return Left;
        }
    }
    ...
    private void parseAttrs(AttributeSet attrs) {
        TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.CustomRatingBar);

        borderColor = arr.getColor(R.styleable.CustomRatingBar_srb_borderColor, getResources().getColor(R.color.orange));
        fillColor = arr.getColor(R.styleable.CustomRatingBar_srb_fillColor, borderColor);
        starBackgroundColor = arr.getColor(R.styleable.CustomRatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = arr.getColor(R.styleable.CustomRatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = arr.getColor(R.styleable.CustomRatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = arr.getColor(R.styleable.CustomRatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = arr.getColor(R.styleable.CustomRatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = arr.getColor(R.styleable.CustomRatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = arr.getInteger(R.styleable.CustomRatingBar_srb_numberOfStars, 5);

        starsSeparation = arr.getDimensionPixelSize(R.styleable.CustomRatingBar_srb_starsSeparation, (int) valueToPixels(4, Dimension.DP));
        maxStarSize = arr.getDimensionPixelSize(R.styleable.CustomRatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = arr.getDimensionPixelSize(R.styleable.CustomRatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = arr.getFloat(R.styleable.CustomRatingBar_srb_stepSize, 0.1f);
        starBorderWidth = arr.getFloat(R.styleable.CustomRatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = arr.getFloat(R.styleable.CustomRatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(arr.getFloat(R.styleable.CustomRatingBar_srb_rating, 0f));
        isIndicator = arr.getBoolean(R.styleable.CustomRatingBar_srb_isIndicator, false);
        drawBorderEnabled = arr.getBoolean(R.styleable.CustomRatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(arr.getInt(R.styleable.CustomRatingBar_srb_gravity, Gravity.Left.id));
    ...
    private void validateAttrs() {
        ...
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            LogUtil.w("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            LogUtil.w("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(CustomRatingBar.this, rating, false);
                }
            }
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
            LogUtil.w("SimpleRatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
            LogUtil.w("SimpleRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
